...
## Upload data 
```{r}
book_spoilers <- book_spoilers[1:100,]
View(book_spoilers)
```
## Data Preparation 
```{r}
...
review_corpus <- data.frame(text=sapply(review_corpus, identity), 
    stringsAsFactors=TRUE)

review_corpus_1 <- review_corpus[1,]

review_corpus_2 <-review_corpus_1 %>% pivot_longer(cols=c(text.1: text.100),
                    names_to='review_sentences',
                    values_to='count')
review_corpus_3 <- review_corpus_2 %>% 
       rename("count" = "cleaned_review_sentence", 
              "review_sentence" = "count")
...
token <- review_3 %>% unnest_tokens(word,review_sentences)
view(token)
